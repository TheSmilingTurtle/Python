...
int main(int argc, char *argv[])
{
    ...
    const double height = ymax;

    const uint32_t Ny = Nx * ymax / (xmax - xmin);
    ...
    {
        cy[ny] = ymax - height * (ny / (double)(Ny - 1));
    }
    ...
    for (uint32_t ny = 0; ny < Ny; ny++)
    {
        for (uint32_t nx = 0; nx < Nx; nx++)
        {
            for (uint32_t n = 0; n < N - 1; n++)
            {
                ...
                if (!(zr[next_idx] * zr[next_idx] + zi[next_idx] * zi[next_idx] <= 2) && (xmin > zr[n] || zr[n] > xmax || -ymax > zi[n] || zi[n] > ymax))
                {
                    for (; n > 0; n--)
                    {
                        if (xmin <= zr[n] && zr[n] <= xmax && -ymax <= zi[n] && zi[n] <= ymax)
                        {
                            ...
                            uint32_t y = ((ymax - fabs(zi[n])) / height - copysignf(0.5 / (double)Ny, zi[n]) ) * (Ny - 1);
                            bd[x + y * Nx]++;
                        }
                    }
                    break;
    ...
    if (xmin <= 0 && 0 <= xmax && -ymax <= 0 && 0 <= ymax)
    {
        uint32_t x = (uint32_t)((-xmin / width + 0.5 / (double)Nx) * (Nx - 1));
        uint32_t y = (uint32_t)((ymax / height + 0.5 / (double)Ny) * (Ny - 1));
    ...
    uint8_t *pic = malloc(Nx * 2 * Ny * sizeof(uint8_t));
    for (uint32_t nx = 0; nx < Nx; nx++)
    {
        for (uint32_t ny = 0; ny < Ny; ny++)
        {
            pic[nx + ny * Nx] = (uint8_t)bd[nx + ny * Nx];
            pic[nx + (2 * Ny - ny - 1) * Nx] = (uint8_t)bd[nx + ny * Nx];
        }
    }
    free(bd);

    FILE *f = fopen("out4-1.pgm", "w");
    fprintf(f, "P5\n%i %i %i\n", Nx, 2 * Ny, UINT8_MAX); // header
    fwrite(pic, Nx * 2 * Ny, 1, f);                      // data
    fclose(f);
