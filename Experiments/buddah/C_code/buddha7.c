...
int main(int argc, char *argv[])
{
    ...
    const uint32_t I = Nx * Ny * 40; // total number of samples
    ...
    {
        struct Args tmp = {bd, trackers + i, large_work, N, Nx, Ny, xmin, xmax, ymin, ymax, width, height};
        thread_args[i] = tmp;
    }
    ...
    {
        struct Args tmp = {bd, trackers + i, small_work, N, Nx, Ny, xmin, xmax, ymin, ymax, width, height};
        thread_args[i] = tmp;
    }
...
// normalize brightness
    ...
    FILE *f = fopen("out.pgm", "w");                 // a very simple image image format to write to
